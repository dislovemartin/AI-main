...
pub async fn handle_request(
    model: web::Data<ExampleModel>,
    req: web::Json<ApiRequest>,
) -> impl Responder {
    info!("Received API request: {:?}", req.input);

    match process_data(&req.input) {
        Ok(processed_input) => match model.predict(&processed_input) {
            Ok(prediction) => HttpResponse::Ok().json(ApiResponse { prediction }),
            Err(e) => {
...
pub fn configure_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(web::resource("/predict").route(web::post().to(handle_request)));
}

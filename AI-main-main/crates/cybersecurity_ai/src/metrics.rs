...
use prometheus::{register_counter_vec, register_histogram_vec, Encoder, CounterVec, HistogramVec, TextEncoder};
use anyhow::Result;

/// Define and register a counter metric.
lazy_static! {
    pub static ref REQUEST_COUNTER: CounterVec = register_counter_vec!(
        "cybersecurity_ai_requests_total",
        "Total number of requests to the Cybersecurity AI service",
        &["service", "status"]
    ).expect("Failed to create REQUEST_COUNTER");

    pub static ref RESPONSE_TIME_HISTOGRAM: HistogramVec = register_histogram_vec!(
        "cybersecurity_ai_response_time_seconds",
        "Histogram for response times of the Cybersecurity AI service",
        &["service"]
